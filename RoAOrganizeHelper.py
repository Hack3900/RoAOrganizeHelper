if(str(input("nothing for hex->text, 'th' for text to hex "))!="th"):
    exec('b = open(r%s,"r").read()' % input("raw hex file "))
    b = list(b)
    c=""
    ...
    while (b!=[]):
        ...
        del b[1]
        del b[0]
    open("RoAOrganizer.RoAText","w+").write(c)
    d = open("RoAOrganizer.RoAText","r")
    for line in d:
        for lin in open(str(line)[0:len(line)-1].lstrip(">-<")+"/config.ini","r"):
            if(lin[0:4]=="name"):
                print(lin[6:-2])
    
else:
    b = open("RoAOrganizer.RoAText","r").read()
    b = list(b)
    c=""
    ...
    while (b!=[]):
        c+=a[b[0]]
        del b[0]
    open("RoAOrganizer.RoAHex","w+").write(c)
    print("The data from \"RoAOrganizer.RoAText\" has been copied to \"RoAOrganizer.RoAHex\"")
input("\npress enter to quit ")
